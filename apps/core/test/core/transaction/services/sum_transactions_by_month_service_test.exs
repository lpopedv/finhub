defmodule Core.Transaction.Services.SumTransactionsByMonthServiceTest do
  ...
  describe "execute/1" do
    test "returns empty list when no transactions in range", %{command: command} do
      assert {:ok, []} = SumTransactionsByMonthService.execute(command)
    end
    ...
    test "sums correctly for a single month", %{user: user, command: command} do
      ...
      assert {:ok, [result]} = SumTransactionsByMonthService.execute(command)
    ...
    test "groups correctly across multiple months", %{user: user, command: command} do
      ...
      assert {:ok, [jan, feb, mar]} = SumTransactionsByMonthService.execute(command)
    ...
    test "orders results by month ascending", %{user: user, command: command} do
      ...
      assert {:ok, results} = SumTransactionsByMonthService.execute(command)
